...
thinking about details. Modern frameworks take away that complexity, but this is
exactly what made the stack worthwhile. I had an occasion to see the state of
...
at most. If I have to add something like analytics I will simply add it to the
request flow.

Of course there were concerns too. For example, raw pixel values fly left and
right in the css for the time being, co-locating styles with templates by using
something like tailwind would be a better experience overall, also, I may have
used next and have mdx configured right away and the list goes on.
...
problem? Yes. Will you have issues with maintaining it? I certainly won't, even
after a year I will just read the code and catch up as there is no hidden magic.
Nor is it a time or place to think about the project complexity and design.
Nobody else will work with me on it, so I don't have to care about that too.

So it is really simple, you may ask? Can you do what has to be done without
pulling your hair out? Yes? Great! Does the change introduce obstacles that will
hinder development in the future? No? Wonderful. Is it quickly deployable?
Probably? [Good enough.](https://www.swyx.io/good-enough) You probably have
something simple on your hands, beside other things.

This blog fulfils that criteria, so let me consider it simple. It came to be
with not much time but a lot of care. Hopefully it serves its purpose well, give
you some material to dig through and inspire you to make your own place on the
internet.

## P.S. It really can be simple

The main code serving this whole website is shown below. External utils and
components do not exceed 100 lines, and are mostly static html or some glue code
for existing libraries. Taking in mind that there is no framework underneath, it
is pretty crazy how much can be done in a modern way with the standard libraries
and a bit of code.
